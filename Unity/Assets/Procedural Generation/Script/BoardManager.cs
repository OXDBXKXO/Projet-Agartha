...
public class BoardManager : MonoBehaviour
{
        ...
        for (int x = -1; x < colum; x++)
        {
            for (int y = -1; y < raw; y++)
            {
                for (int z = -1; z < zaxis; z++)
                {
                    ...
                    if (x == -1 || x == colum || y == -1 || y == raw||z ==-1||z ==zaxis)
                    {
                        Instence = outerwall[Random.Range(0, outerwall.Length)];
                    }
                    GameObject inst = Instantiate(Instence, new Vector3(x, y, z), Quaternion.identity) as GameObject;
    ...
    public void Sscene(int lvl)
    {
        ...
        layatran(Foodt, lifedrop.mini, lifedrop.maxi);
        int ennemyc = (int)Mathf.Log(lvl, 2f);
        layatran(ennemy, ennemyc, ennemyc);
        Instantiate(exit, new Vector3(colum - 1, raw - 1, zaxis-1), Quaternion.identity);
    }
}
